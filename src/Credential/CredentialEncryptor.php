...
final class CredentialEncryptor {
	...
	public function __construct( ?string $key = null ) {
		...
		if ( SODIUM_CRYPTO_SECRETBOX_KEYBYTES !== strlen( $this->key ) ) {
			throw new RuntimeException( 'The credential encryption key has an invalid length.' );
		}
	}
	...
	public function decrypt( string $encoded ): string {
		...
		if ( false === $payload || strlen( $payload ) <= SODIUM_CRYPTO_SECRETBOX_NONCEBYTES ) {
			throw new UnexpectedValueException( 'The encrypted credential is invalid.' );
		}
		...
		if ( false === $plaintext ) {
			throw new UnexpectedValueException( 'Credential authentication failed.' );
		}
	...
	private function derive_key(): string {
		return hash_hkdf(
			...
			'od-wordpress-monitor-credentials'
		);
	}
}

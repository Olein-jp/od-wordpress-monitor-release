...
class Bootstrap {
	...
	public function _pre_set_site_transient_update_plugins( $transient ) {
		...
		$current_version = $this->_sanitize_version( $current['Version'] );
		$new_version     = $this->_sanitize_version( $response->tag_name );

		if ( version_compare( $current_version, $new_version, '<' ) && ! $response->package ) {
			return $transient;
		}
	...
	public function _upgrader_process_complete( $upgrader_object, $hook_extra ) {
		...
		if ( in_array( $this->plugin_name, $plugins, true ) ) {

...
登録サイトの名称・URL・認証情報は管理画面から編集でき、監視を一時停止・再開できます。一時停止中は定期チェックと通知を行わず、既存の履歴は保持されます。再開後の現在状態は次回チェックまで未確認です。管理画面からの物理削除はできません。
...
- サイトヘルスに「おすすめの改善」だけがある場合は監視状態を「正常」とし、推奨項目の詳細表示は維持します。致命的な問題の判定は従来どおりです。
- サイト詳細画面の移動・編集・月次レポートのリンクを横並びのボタンにし、月次レポートのCSVボタンを表の右下に移しました。

### 1.1.0

- サイトの名称・URL・認証情報の編集と、監視の一時停止・再開に対応しました。再開後の初回チェックで障害を確認した場合も通知します。
- サイトごとに通知種別と既存チャネルの利用可否を設定できるようにしました。未設定のサイトは従来どおり全体設定を継承します。
- 保存済み履歴からサイト別の月次レポートを表示・CSV出力できるようにしました。欠測や保持期間外はデータ不足として示し、稼働率として扱いません。
- 管理メニューの簡素化を任意で有効化できるようにしました。初期状態では全メニューを表示します。

### 1.0.11

- 通知先を保存済みでもテスト通知ボタンが無効のままになる問題を修正しました。
